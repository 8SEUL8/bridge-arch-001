...
DEFAULT_CONFIG = {
    ...
    "quorum": {
        ...
        "binding_classes": ["CONSTITUTIONAL", "ORDINARY"],
        "absolute_floor_binding": 3,
        "non_quorate_label": "NON-QUORATE OUTPUT — ADVISORY ONLY — NOT BINDING",
    },
    "api": {
        "max_retries": 3,
        "retry_delay": 30,
...
def _member_alias_map() -> dict:
    aliases = {}
    for key, cfg in PROVIDERS.items():
        display = cfg["name"]
        base_name = display.split("(")[0].strip()
        aliases[key.lower()] = key
        aliases[display.lower()] = key
        aliases[base_name.lower()] = key
    return aliases


def _normalize_member_list(values) -> set:
    if not values:
        return set()
    if isinstance(values, str):
        values = [values]
    aliases = _member_alias_map()
    normalized = set()
    for value in values:
...
def _response_is_substantive(response: str) -> bool:
    if not response:
        return False
    stripped = response.strip()
    upper = stripped.upper()
    if upper.startswith("[ERROR]") or upper.startswith("[UNAVAILABLE]"):
        return False
    return len(stripped) > 0


def compute_restoration_metadata(agenda_item: dict) -> dict:
    restoration_of = agenda_item.get("restoration_of") or agenda_item.get("ratifies_session")         or agenda_item.get("prior_non_quorate_session") or agenda_item.get("reconsideration_of")
    mode = "NONE"
    if agenda_item.get("restoration_of"):
        mode = "RESTORATION"
    elif agenda_item.get("ratifies_session"):
        mode = "RATIFICATION"
    elif agenda_item.get("prior_non_quorate_session") or agenda_item.get("reconsideration_of"):
    ...
    return {
        ...
        "restoration_notes": agenda_item.get("restoration_notes", ""),
        "requires_re_ratification": bool(restoration_of),
    }


def compute_quorum_metadata(agenda_item: dict, available_providers: list,
                            vote_responses: dict, config: dict) -> dict:
    quorum_cfg = (config or {}).get("quorum", {})
    thresholds = quorum_cfg.get("thresholds", {})
    decision_class = str(
        agenda_item.get("decision_class")
        or quorum_cfg.get("decision_class_default", "ORDINARY")
    ).upper()
    threshold_ratio = float(thresholds.get(decision_class, thresholds.get("ORDINARY", 0.75)))
    binding_classes = {c.upper() for c in quorum_cfg.get("binding_classes", ["CONSTITUTIONAL", "ORDINARY"])}
    absolute_floor_binding = int(quorum_cfg.get("absolute_floor_binding", 3))

    recused = _normalize_member_list(agenda_item.get("recused_members", []))
    vacant = _normalize_member_list(agenda_item.get("vacant_members", []) or agenda_item.get("vacant_seats", []))
    ...
    for provider, cfg in PROVIDERS.items():
        if provider in vacant:
            status = "VACANT"
        elif provider in recused:
            status = "RECUSED"
        elif provider not in available_set:
            status = "UNAVAILABLE"
        elif _response_is_substantive(vote_responses.get(provider, "")):
            status = "PRESENT"
            participating_members.append(cfg["name"])
        else:
            status = "FAILED"
            missing_members.append(cfg["name"])
        seat_status[cfg["name"]] = status

    for provider, cfg in PROVIDERS.items():
        if seat_status[cfg["name"]] in {"UNAVAILABLE", "FAILED"}:
            if cfg["name"] not in missing_members:
                missing_members.append(cfg["name"])

    seated_members = [cfg["name"] for provider, cfg in PROVIDERS.items() if provider not in vacant]
    eligible_members = [cfg["name"] for provider, cfg in PROVIDERS.items() if provider not in vacant and provider not in recused]
    participating_count = len(participating_members)
    eligible_count = len(eligible_members)
    required_count = 0
    if eligible_count > 0:
        required_count = max(1, int((eligible_count * threshold_ratio) + 0.999999999))

    binding_authorized = False
    non_quorate = False
    if decision_class in binding_classes:
        required_count = max(required_count, min(absolute_floor_binding, eligible_count or absolute_floor_binding))
        binding_authorized = participating_count >= required_count and eligible_count > 0
        non_quorate = not binding_authorized
    else:
        binding_authorized = False
        non_quorate = True
    ...
    return {
        ...
        "total_seats": len(PROVIDERS),
        "seated_count": len(seated_members),
        "eligible_count": eligible_count,
        "participating_count": participating_count,
        "binding_authorized": binding_authorized,
        "non_quorate": non_quorate,
        "non_quorate_label": quorum_cfg.get(
            "non_quorate_label",
            "NON-QUORATE OUTPUT — ADVISORY ONLY — NOT BINDING",
        ),
        "council_state": council_state,
        "participating_members": participating_members,
        "missing_members": missing_members,
        "recused_members": [PROVIDERS[p]["name"] for p in recused],
        "vacant_members": [PROVIDERS[p]["name"] for p in vacant],
...
def run_deliberation(proposal: str, agenda_item: dict, providers: list,
 ...
 # ── Phase 0: Independent Research (Evidence Multi-Channeling) ──
    ...
    # Tally + quorum/restoration assessment
    result = tally_votes(p3)
    quorum_meta = compute_quorum_metadata(agenda_item, providers, p3, config)
    restoration_meta = compute_restoration_metadata(agenda_item)
    result["quorum"] = quorum_meta
    result["restoration"] = restoration_meta
    result["binding_outcome"] = result["outcome"] if quorum_meta["binding_authorized"] else "NON_QUORATE_ADVISORY"

    record.add_entry(
        "quorum_assessment",
        "SYSTEM",
        json.dumps(quorum_meta, ensure_ascii=False),
        quorum_meta,
    )
    if restoration_meta.get("mode") != "NONE":
        record.add_entry(
            "restoration_metadata",
            "SYSTEM",
            json.dumps(restoration_meta, ensure_ascii=False),
            restoration_meta,
        )
    record.add_entry("vote_tally", "SYSTEM", json.dumps(result, ensure_ascii=False), result)
    log.info(
        f"RESULT: {result['outcome']} / binding={result['binding_outcome']} — {result['details']}"
    )
    ...
    record.add_entry("session_close", "SYSTEM",
        f"Outcome: {result['outcome']}. Binding: {result['binding_outcome']}. "
        f"Council state: {result['quorum']['council_state']}. Chain valid: {record.verify_chain()}")
...
def save_record(record_dict: dict, log):
    ...
    quorum_entry = next((e for e in entries if e.get("phase") == "quorum_assessment"), None)
    restoration_entry = next((e for e in entries if e.get("phase") == "restoration_metadata"), None)
    vote_tally_entry = next((e for e in entries if e.get("phase") == "vote_tally"), None)
    vote_meta = vote_tally_entry.get("metadata", {}) if vote_tally_entry else {}
    quorum_meta = quorum_entry.get("metadata", {}) if quorum_entry else vote_meta.get("quorum", {})
    restoration_meta = restoration_entry.get("metadata", {}) if restoration_entry else vote_meta.get("restoration", {})

    input_payload = {}
    if input_hash_entry:
        try:
            input_payload = json.loads(input_hash_entry.get("content", "{}"))
    ...
    def write_readable_markdown(path: str):
        with open(path, 'w', encoding='utf-8') as f:
            ...
            if vote_meta:
                f.write(f"**Outcome:** {vote_meta.get('outcome', 'N/A')}\n")
                f.write(f"**Binding Outcome:** {vote_meta.get('binding_outcome', vote_meta.get('outcome', 'N/A'))}\n")
            if quorum_meta:
                f.write(f"**Decision Class:** {quorum_meta.get('decision_class', 'ORDINARY')}\n")
                f.write(f"**Council State:** {quorum_meta.get('council_state', 'UNKNOWN')}\n")
                f.write(f"**Quorum:** {quorum_meta.get('participating_count', 0)}/{quorum_meta.get('eligible_count', 0)} participating (required: {quorum_meta.get('required_count', 'N/A')})\n")
                f.write(f"**Binding Authorized:** {quorum_meta.get('binding_authorized', False)}\n")
                if quorum_meta.get('non_quorate'):
                    f.write(f"**Record Label:** {quorum_meta.get('non_quorate_label', 'NON-QUORATE OUTPUT — ADVISORY ONLY — NOT BINDING')}\n")
                if quorum_meta.get('missing_members'):
                    f.write("**Missing Members:** " + ", ".join(quorum_meta.get('missing_members', [])) + "\n")
                if quorum_meta.get('recused_members'):
                    f.write("**Recused Members:** " + ", ".join(quorum_meta.get('recused_members', [])) + "\n")
                if quorum_meta.get('vacant_members'):
                    f.write("**Vacant Members:** " + ", ".join(quorum_meta.get('vacant_members', [])) + "\n")
            if restoration_meta and restoration_meta.get('mode') != 'NONE':
                f.write(f"**Restoration Mode:** {restoration_meta.get('mode')}\n")
                if restoration_meta.get('restoration_of'):
                    f.write(f"**Restoration Of:** {restoration_meta.get('restoration_of')}\n")
                if restoration_meta.get('restoration_reason'):
                    f.write(f"**Restoration Reason:** {restoration_meta.get('restoration_reason')}\n")
            if input_hash_entry:
                f.write(f"**Input Hash:** `{input_hash_entry.get('metadata', {}).get('input_hash', 'N/A')}`\n")
                f.write(
                    "**Input Scope:** "
